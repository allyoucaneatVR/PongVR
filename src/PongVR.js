...
function load(webVR, cardboard, distortion){
    ...
    window.setTimeout(function(){
        init(cardboard, distortion);
        document.getElementById('vrone-settings').style.display = 'block';
        document.getElementById('loading-screen').style.display = 'none';
    }, 10);
}
function init(cardboard, distortion) {
...
var serverO3Ds = {};
var cameraConfig = new VROne.CameraModifier();
...
function initScene(){
    ...
    function createO3Ds(){
        ...
        bodyO3Ds = new VROne.OBJLoader(path + "obj/body.obj");
        body = getNewBody();
        ...
        platform1 = new VROne.Geometry.Box(1, 1, 1);
        ...
        platform2 = new VROne.Geometry.Box(1, 1, 1);
        ...
        ball = new VROne.OBJLoader(path + "obj/ball.obj")[0];
        ball.scale.x = .2;
        ball.scale.y = .2;
        ball.scale.z = .2;
        ...
        torus = new VROne.OBJLoader(path + "obj/Torus.obj").Torus;
        ...
        var setupTorusShader = function() {
            ...
            var poolNorm = new VROne.Vector3();
            var radius = 4 * scale;
            var torusCount = 15;
            var torusNormal = torus.rotation.getRotatedPoint(new VROne.Vector3(0, 0, -1));
        ...
        scoreboard0 = new VROne.OBJLoader(path + "obj/scoreboard3.obj")[0];
        ...
        scoreboard1 = new VROne.OBJLoader(path + "obj/scoreboard3.obj")[0];
        ...
        cursor = new VROne.OBJLoader(path + "obj/cursor.obj")[0];
        cursor.position.z = -2;
        ...
        cursor.scale = new VROne.Vector3(0.05,0.05,1.0);
        ...
        numbers = [];
        for(var i = 0; i < 5; i++){
            numbers[i] = new VROne.OBJLoader(path + "obj/number.obj")[0];
        ...
        rdy = new VROne.OBJLoader(path + "obj/rdy.obj")[0];
        ...
        function initParticles(){
            var particles0 = new VROne.OBJLoader(path + "obj/ico.obj")[0];
            var particles1 = new VROne.OBJLoader(path + "obj/icorandom2.obj")[0];
            ...
            var x, y, z, scale;
            icoSystem0 = new VROne.ParticleSystem(scene, particles1, 200, 300000);
            icoSystem1 = new VROne.ParticleSystem(scene, particles0, 200, 300000);
            for(var i=0; i<icoSystem0.particles.length; i++){
                ...
                icoSystem0.particles[i].position = new VROne.Vector3(x, -50, z);
                icoSystem1.particles[i].position = new VROne.Vector3(x, -50, z);

                y = (Math.random())*0.0015;
                icoSystem0.particles[i].velocity = new VROne.Vector3(0, y, 0);
                icoSystem1.particles[i].velocity = new VROne.Vector3(0, y, 0);

                scale = 5+Math.random()*5;
                icoSystem0.particles[i].scale = new VROne.Vector3(scale, scale, scale);
                icoSystem1.particles[i].scale = new VROne.Vector3(scale-2, scale-2, scale-2);

                var lifetime = 300000+Math.random()*100000;
                icoSystem0.particles[i].lifetime = lifetime;
                icoSystem1.particles[i].lifetime = lifetime;

                icoSystem0.particles[i].rotationAngle = new VROne.Vector3(
                    Math.random()*0.0001,
                    Math.random()*0.0001,
                    Math.random()*0.0001);
                icoSystem1.particles[i].rotationAngle = new VROne.Vector3(
                    Math.random()*0.0003,
                    Math.random()*0.0003,
                    Math.random()*0.0003);
        ...
        function initBalloons(){
            var balloon = new VROne.OBJLoader(path + "obj/balloon.obj")[0];
            ...
            balloons = new VROne.ParticleSystem(scene, balloon, 30, 0);
            for(var i=0; i<balloons.particles.length; i++){
                balloons.particles[i].position = new VROne.Vector3(
                    (Math.random()*2-1)*6,
                    5-Math.random()*5,
                    (Math.random()*2-1)*6);
                balloons.particles[i].gravity = 0.00000000001;
                balloons.particles[i].gravityExponent = 2;
                balloons.particles[i].scale = new VROne.Vector3(.3,.3,.3);
        ...
        //audioContext = new VROne.AudioContext();
//        sound = new VROne.Sound("audio/01.wav");
//        sound.loop = true;
//        sound.is3D = true;
//        sound.volume = 1;
//        sound.position.x = -1;
//        sound.orientationFront = new VROne.Vector3(0,0,1);
...
function getNewBody(){
    if(mobileVR){
        var bodySimple = new VROne.OBJLoader(path + "obj/body_simple.obj")[0];
    ...
    for(var attr in bodyO3Ds){
        if(isNaN(attr)){
            o3Ds[attr] = VROne.OBJLoader.prototype.copyOBJO3D(bodyO3Ds[attr]);
        }
    }
    ...
    var boneMovement = function(){
        if(this.animationActive){
            ...
            //end
            if(p >= 1){
                VROne.Quaternion.prototype.copyToQuaternion(this.rotateTo, this.rotation);
    ...
    for(var bodyPart in body.bodyParts){
        ...
        bone.rotateTo = new VROne.Quaternion();
        bone.duration = 0;
    ...
    body.lastPos = new VROne.Vector3();
    body.isMoving = false;
    body.stopTime = 0;
    body.onUpdate = function(){
        ...
        if(VROne.Vector3.prototype.distance(lP, cP) > 0){
            if(!body.isMoving){
                body.isMoving = true;
            }
            lP.x = cP.x;
...
//                console.log("Adding Sphere");
                //o3D = new VROne.Geometry.Sphere(0.2).getO3D();
                o3D = ball;
            }
...
//    console.log("creating Player " + id);
    var headP = new VROne.CubeTexture3D_2();
    headP.scale = new VROne.Vector3(0.3, 0.3, 0.3);
...
function runScoreAnimation(){
    if(VROne.KeyboardHandler.isKeyDown("G")){   // TODO: remove
        if(!gKeyPressed) {
            //score1++;
            //score2++;
            //console.log("score!");
