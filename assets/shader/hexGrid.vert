...
void main(void) {
	vFragmentCoord = vec2(	// coordinates as (x,y), 0<=x<=1, 0<=y<=1
		(abs(aVertexPosition.x)+aVertexPosition.x)/(abs(aVertexPosition.x)*2.0),
		(abs(aVertexPosition.z)+aVertexPosition.z)/(abs(aVertexPosition.z)*2.0));
